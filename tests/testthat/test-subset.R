...
test_that("Classes of pruned MicrobiomeExperiment and its components are as expected", {
	GP3 <- prune_samples(keepNames, GP)
	expect_that(nsamples(GP3), is_identical_to(3L))
	expect_that(GP3, is_a("MicrobiomeExperiment"))
	...
	GPnoSD  <- MicrobiomeExperiment(otu_table(GP), tax_table(GP))
...
test_that("filter_taxa gives correct, reliable logicals and pruning", {
	...
	expect_that(ent.trim, is_a("MicrobiomeExperiment"))
