...
library("MicrobiomeExperiment"); packageVersion("MicrobiomeExperiment")
...
test_that("Classes of merged MicrobiomeExperiment objects are as expected", {
	...
	expect_that(mGP, is_a("MicrobiomeExperiment"))
})
...
test_that("merge_MicrobiomeExperiment: Break apart GP based on human-association,
	...
	GP1 <- merge_MicrobiomeExperiment(h0, h1)
...
test_that("the tax_table slot is identical whether tax_glom()ed by itself or as component", {
	...
	expect_that(n1<-tax_glom(GP.chl, "Family"), is_a("MicrobiomeExperiment"))
...
test_that("tax_glom() handles clearly agglomeration to one taxa", {
	...
	expect_that(n1, is_a("MicrobiomeExperiment"))
...
test_that("tax_glom() can handle even the highest rank glom", {
  ...
  expect_is(gpk, "MicrobiomeExperiment")
...
test_that("prune_taxa() properly handles standard-cases", {
	# throws warning, and NULL-tre
	expect_that(n1 <- prune_taxa(taxa_names(GP.chl)[1:5], GP.chl), is_a("MicrobiomeExperiment"))
	...
	L2[1:5] <- TRUE
	expect_that(n2 <- prune_taxa(L2, GP.chl), is_a("MicrobiomeExperiment"))
...
test_that("merge_taxa() properly handles standard-cases", {
	expect_that(n1 <- merge_taxa(GP.chl, c("24341", "579085")), is_a("MicrobiomeExperiment"))
  ...
  # Perform the merge of `OTUIDs`, and check the resulting class while at it.
	expect_is(n2 <- merge_taxa(GP.chl, OTUIDs), "MicrobiomeExperiment")
  ...
  # Merge again, but only use the tax_table. No counts changes default retained to first in vector
	...
	expect_that(n3 <- merge_taxa(GP.chl, eqtaxa=OTUIDs, archetype=OTUIDs[3]), is_a("MicrobiomeExperiment"))
...
test_that("merge_taxa() replaces disagreements in taxonomy with NA", {
  ...
  expect_is(n7 <- merge_taxa(GP20f, OTUIDs), "MicrobiomeExperiment")
	...
	tax_table(GP20f)[1, length(rank_names(GP20f))] <- "species_MicrobiomeExperiment_test"
  OTUIDs = c("951", biggestOTU)
	biggestOTU = names(which.max(taxa_sums(GP20f)[OTUIDs]))
	expect_is(n8 <- merge_taxa(GP20f, OTUIDs), "MicrobiomeExperiment")
...
test_that("merge_taxa() properly handles different types and orders of taxa specified by the eqtaxa and archetype arguments, and also handles refseq data", {
  ...
  expect_that(length(getslots.MicrobiomeExperiment(rs1)), equals(5L))
  expect_that(length(getslots.MicrobiomeExperiment(rs2)), equals(5L))
  expect_that(length(getslots.MicrobiomeExperiment(rs3)), equals(5L))
  expect_that(length(getslots.MicrobiomeExperiment(rs4)), equals(5L))
