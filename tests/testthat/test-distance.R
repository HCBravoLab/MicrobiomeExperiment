...
################################################################################
# Define the test-example MicrobiomeExperiment dataset. A random subsample from Global Patterns
treeFile = system.file("extdata", "GP_tree_rand_short.newick.gz", package="MicrobiomeExperiment")
GP500File = system.file("extdata", "GP_otu_table_rand_short.txt.gz", package = "MicrobiomeExperiment")
...
gp500_uuf = read.csv(system.file("extdata", "gp500-uuf.csv", package = "MicrobiomeExperiment"), header = FALSE, fill = TRUE)
gp500_wuf = read.csv(system.file("extdata", "gp500-wuf.csv", package = "MicrobiomeExperiment"), header = FALSE, fill = TRUE)
gp500_wufu = read.csv(system.file("extdata", "gp500-wufu.csv", package = "MicrobiomeExperiment"), header = FALSE, fill = TRUE)
...
test_that("Test accurate dispatch for other distances", {
  ...
  # Test for all vegdist, MicrobiomeExperiment object
  expect_equal(
  ...
  expect_equal(
    ...
    check.attributes = FALSE, tolerance = tol)
  # Test for all betadiver, MicrobiomeExperiment object
  expect_equal(
  ...
  expect_equal(
    ...
    check.attributes = FALSE, tolerance = tol) 
  # Test for all dist, MicrobiomeExperiment object
  expect_equal(
  ...
  # MicrobiomeExperiment object
  expect_equal(
    MicrobiomeExperiment:::JSD(GP500),
    distance(GP500, "jsd"),
    check.attributes = FALSE, tolerance = tol)
  # OTU table
  expect_equal(
    MicrobiomeExperiment:::JSD(otu_table(GP500)),
