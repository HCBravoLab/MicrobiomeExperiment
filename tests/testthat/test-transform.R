################################################################################
# Use testthat to test MicrobiomeExperiment transformation functions/methods
################################################################################
library("MicrobiomeExperiment"); library("testthat")
# # # # TESTS!
#set.seed(8888)
...
test_that("Can transform_sample_counts on MicrobiomeExperiment with either orientation", {
  data("esophagus")
  eso1 = eso2 = NULL
  try(eso1 <- transform_sample_counts(esophagus, rank), TRUE)
  try(eso2 <- transform_sample_counts(t(esophagus), rank), TRUE)
  expect_false(is.null(eso1), "eso1 is NULL, valid MicrobiomeExperiment construction failed.")
  expect_false(is.null(eso2), "eso2 is NULL, valid MicrobiomeExperiment construction failed.")
  expect_is(eso1, "MicrobiomeExperiment", "class of eso1 is not MicrobiomeExperiment")
  expect_is(eso2, "MicrobiomeExperiment", "class of eso2 is not MicrobiomeExperiment")
...
test_that("Test transform_sample_counts edge-cases", {
	...
	# In this case a "MicrobiomeExperiment" object with just an OTU table. 
	# Really, this should just be considered an object of class "otu_table".
	#expect_is(eso1otu, "MicrobiomeExperiment", "pruned-to-1-OTU esophagus not MicrobiomeExperiment")
	eso1otu = otu_table(eso1otu)
	expect_is(eso1otu, "otu_table")
	expect_equal(dim(otu_table(eso1otu)), c(1L, nsamples(esophagus)))	
	# Test eso1sam
	expect_is(eso1sam, "MicrobiomeExperiment", "pruned-to-1-sample esophagus not MicrobiomeExperiment")
	...
	expect_is(eso1samrank, "MicrobiomeExperiment")
	expect_is(eso1samrankt, "MicrobiomeExperiment")
