################################################################################
# Use testthat to test MicrobiomeExperiment constructor and other class internals.
################################################################################
...
################################################################################
test_that("MicrobiomeExperiment: Building a MicrobiomeExperiment-object when tree contains extra quotes, still works.", {
	data("esophagus")
	...
	esophagus2 = MicrobiomeExperiment(tree, otu_table(esophagus)) 
	...
	# Now try to "rebuild" using the quote-containing 
	GP2 = MicrobiomeExperiment(tree, otu_table(GP), tax_table(GP), sample_data(GP)) 
...
# 	expect_that(identical(taxa_names(refseq(e1)), new_taxa_names), is_false())	
	...
	expect_that(identical(length(MicrobiomeExperiment:::intersect_taxa(e1)), nunchained), is_true())
	e2 = prune_taxa(MicrobiomeExperiment:::intersect_taxa(e1), e1)
	...
	## Okay so that worked. Now we test if the intersection functions behave
	expect_that(identical(length(MicrobiomeExperiment:::intersect_samples(e1)), nunchained), is_true())
	e2 = prune_samples(MicrobiomeExperiment:::intersect_samples(e1), e1)
...
test_that("Test ordering", {
	...
	e3 = MicrobiomeExperiment(OTU, tree)
	...
	e4 = MicrobiomeExperiment:::index_reorder(e4, "both")
