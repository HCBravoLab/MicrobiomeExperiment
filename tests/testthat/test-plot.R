...
################################################################################
library("MicrobiomeExperiment"); library("testthat"); library("ggplot2")
data("GlobalPatterns")
...
GP.tr  <- phy_tree(GP)
# GP <- MicrobiomeExperiment(otu_table(GP), GP.tr)
GP.otu <- otu_table(GP)
...
# the formula specified the MicrobiomeExperiment-data
...
# Create (merged) MicrobiomeExperiment-class GP, and run comparisons
test_that("all 4 plot_ordination type options result in valid ggplot2 object", {
	GP <- merge_MicrobiomeExperiment(GP.otu, GP.tr)
...
test_that("plot_ordination: The justDF=TRUE option returns a data.frame", {
  # Make GP a MicrobiomeExperiment object with only tree (no ordination co-variables to plot)
	GP <- merge_MicrobiomeExperiment(GP.otu, GP.tr)
...
test_that("plot_ordination: When variables are present or not, shape SamplyType", {
	# GP <- merge_MicrobiomeExperiment(GP.otu, GP.tr, GP.sd, GP.tax)
...
################################################################################
test_that("psmelt correctly handles MicrobiomeExperiment data with NULL components, and OTU tables", {
  data("GlobalPatterns")
  GP = prune_taxa(names(sort(taxa_sums(GlobalPatterns), TRUE)[1:50]), GlobalPatterns)
  # The objects with NULL components
  GPS = MicrobiomeExperiment(otu_table(GP), sample_data(GP), phy_tree(GP))
  GPT = MicrobiomeExperiment(otu_table(GP), tax_table(GP), phy_tree(GP))
  GPTr = MicrobiomeExperiment(otu_table(GP), phy_tree(GP))
  GPN = otu_table(GP)
